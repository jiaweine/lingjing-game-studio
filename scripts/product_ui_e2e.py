...
window.fetch=async function(url,opt={}){
  ...
  if(u.match(/\/api\/conversations\/[^/]+\/messages$/)&&method==='POST'){
    ...
    setTimeout(()=>{
      const events=[
        {id:1,type:'progress',payload:{step:'读取任务上下文',detail:'已关联当前素材，正在建立执行上下文',percent:12,job_id:'job-e2e'}},
        {id:2,type:'progress',payload:{step:'定位异常窗口',detail:'正在锁定掉血变化与资源变化发生的时间段',percent:36,job_id:'job-e2e'}},
        {id:3,type:'progress',payload:{step:'尝试稳定复现',detail:'正在用一致初始条件检查不同触发路径',percent:58,job_id:'job-e2e'}},
        {id:4,type:'progress',payload:{step:'交叉核对证据',detail:'正在对照截图、日志与重复结果',percent:78,job_id:'job-e2e'}},
        {id:5,type:'progress',payload:{step:'验证结论',detail:'已完成复核，正在整理证据与下一步',percent:100,job_id:'job-e2e'}},
