...
def _case_annotation_state(
    ...
    *,
    ...
    candidates_ok = (
        bool(candidates)
        and len(candidates) == len(candidate_set)
        and not (candidate_set - known_asset_ids)
    )
    ...
    relevant_ok = (
        ...
        and not (relevant_ids - candidate_set)
    )
    forbidden = {
        _clean(value)
    ...
    return {
        ...
        "semantic_hard_negative_count": max(
            0, len(candidate_set - relevant_ids - forbidden)
        ),
        "temporal": temporal,
    }
...
def annotation_readiness_report(workspace: dict[str, Any]) -> dict[str, Any]:
    ...
    asset_modality_counts: Counter[str] = Counter()
    ...
    target_modality_cases: Counter[str] = Counter(
        {kind: 0 for kind in sorted(_ALLOWED_MODALITIES)}
    )
    temporal_cases = 0
    ...
    protocol_coverage_ready = (
        deficits["cases"] == 0
        and deficits["unique_source_groups"] == 0
        and not any(deficits["target_modalities"].values())
        and deficits["temporal_cases"] == 0
        and deficits["scope_negative_cases"] == 0
    )
