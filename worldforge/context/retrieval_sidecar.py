...
class MultimodalRetrievalClient:
    ...
    def _descriptor(asset: dict[str, Any]) -> dict[str, Any]:
        ...
        keep_meta = {
            key: meta.get(key)
            for key in (
                "kind", "duration", "width", "height", "has_audio", "sample_rate",
                "channels", "chars", "lines", "keyframes", "keyframe_times",
                "build", "branch", "commit",
            )
    ...
    async def rank(self, query: str, assets: list[dict[str, Any]]) -> MultimodalRetrievalResult:
        ...
        try:
            ...
            return MultimodalRetrievalResult(
                ...
                backend=(str(data.get("backend")) if data.get("backend") else None),
...
def apply_retrieval_hits(
    ...
    *,
    ...
    baseline_ranks = [
        int((asset.get("meta", {}) or {}).get("_context", {}).get("rank") or 0)
        for asset in assets
    ]
