...
class CombatAnalyst(CouncilAgent):
    ...
    def score(self, action, state, belief, goal):
        ...
        value = (
            ...
            if action == ActionKind.HEAVY_ATTACK
            else (2.2 if state.energy >= 2 else -9)
            if action == ActionKind.CAST
            else 1.7
        ...
        value += (
            3.2
            if enemy_ratio < .42
            and action in (ActionKind.HEAVY_ATTACK, ActionKind.CAST)
            else 0.0
        )
...
class RiskAnalyst(CouncilAgent):
    ...
    def score(self, action, state, belief, goal):
        ...
        value = (
            ((1 - hp) * 2.5 + state.threat * 1.7)
            if action == ActionKind.DEFEND
            else ((1 - hp) * 7)
            if action == ActionKind.HEAL
            else 2.8
            if action == ActionKind.SCOUT and belief.uncertainty > .45
            else -3.8
            if action in (ActionKind.HEAVY_ATTACK, ActionKind.CAST) and hp < .3
            else (2.0 if hp < .16 else -5)
            if action == ActionKind.RETREAT
            else 0.0
        )
        return (
            value,
            f"hp={hp:.2f}, threat={state.threat:.2f}, uncertainty={belief.uncertainty:.2f}",
        )
...
class EconomyAnalyst(CouncilAgent):
    ...
    def score(self, action, state, belief, goal):
        value = (
            (2.2 if state.armor < 6 else .2)
            if action == ActionKind.BUY_ARMOR
            else (1.8 if state.attack < 22 else .2)
            if action == ActionKind.BUY_BLADE
            else (1.5 if state.gold < 22 else .4)
            if action == ActionKind.FARM
            else 0.0
        )
        value -= (
            1.3 if "exploit-test" in state.tags and action == ActionKind.FARM else 0.0
        )
...
class ProgressAnalyst(CouncilAgent):
    ...
    def score(self, action, state, belief, goal):
        ...
        value = (
            3.4 + 5 / remaining
            if action in (ActionKind.ATTACK, ActionKind.HEAVY_ATTACK, ActionKind.CAST)
            else -3
            if action == ActionKind.FARM and remaining < 6
            else -1
            if action == ActionKind.SCOUT and state.tick > 4
            else 0.0
        )
...
class AdaptivePlanner:
    ...
    def _epistemic_adjustment(action, scores, state, belief):
        """Turn internal disagreement into exploration value and irreversible-risk friction.

        This is deliberately model-independent. When specialists disagree under an
        uncertain world state, information-gathering actions become more valuable;
        high-threat irreversible actions are discounted until evidence converges.
        """
        disagreement = pstdev(scores) if len(scores) > 1 else 0.0
        tension = min(4.0, disagreement) * belief.uncertainty
        if action == ActionKind.SCOUT:
            return 1.15 + tension * 1.35
        if action == ActionKind.DEFEND:
            return tension * state.threat * .22
        if action in (ActionKind.HEAVY_ATTACK, ActionKind.CAST):
            return -tension * (.28 + state.threat * .42)
        if action == ActionKind.RETREAT:
            return -tension * .12 if state.threat < .7 else tension * .08
        return -tension * state.threat * .08
